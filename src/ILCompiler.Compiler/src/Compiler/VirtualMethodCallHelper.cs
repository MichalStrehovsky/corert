...
namespace ILCompiler
{
    public static class VirtualMethodSlotHelper
    {
        ...
        public static int GetVirtualMethodSlot(NodeFactory factory, MethodDesc method, TypeDesc implType, bool countDictionarySlots = true)
        {
            ...
            if (method.CanMethodBeInSealedVTable(factory.DevirtualizationManager) && !factory.IsCppCodegenTemporaryWorkaround)
            {
                ...
                {
                    if (virtualSlots[slot].CanMethodBeInSealedVTable(factory.DevirtualizationManager))
                        continue;
                    numVTableSlots++;
                }
            ...
            else
            {
                ...
                for (int slot = 0; slot < virtualSlots.Count; slot++)
                {
                    if (virtualSlots[slot].CanMethodBeInSealedVTable(factory.DevirtualizationManager) && !factory.IsCppCodegenTemporaryWorkaround)
                    {
        ...
        private static int GetNumberOfBaseSlots(NodeFactory factory, TypeDesc owningType, bool countDictionarySlots)
        {
            ...
            while (baseType != null)
            {
                ...
                foreach (var vtableMethod in baseVirtualSlots)
                {
                    ...
                    if (vtableMethod.CanMethodBeInSealedVTable(factory.DevirtualizationManager) && !factory.IsCppCodegenTemporaryWorkaround)
                        continue;
                    baseSlots++;
                }
