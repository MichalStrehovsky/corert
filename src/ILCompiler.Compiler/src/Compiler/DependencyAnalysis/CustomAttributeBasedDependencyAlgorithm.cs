...
namespace ILCompiler.DependencyAnalysis
{
    ...
    {
        public static void AddDependenciesDueToCustomAttributes(ref DependencyList dependencies, ILScanNodeFactory factory, EcmaMethod method)
        {
        ...
        public static void AddDependenciesDueToCustomAttributes(ref DependencyList dependencies, ILScanNodeFactory factory, EcmaType type)
        {
        ...
        public static void AddDependenciesDueToCustomAttributes(ref DependencyList dependencies, ILScanNodeFactory factory, EcmaField field)
        {
        ...
        public static void AddDependenciesDueToCustomAttributes(ref DependencyList dependencies, ILScanNodeFactory factory, EcmaAssembly assembly)
        {
        ...
        private static void AddDependenciesDueToCustomAttributes(ref DependencyList dependencies, ILScanNodeFactory factory, EcmaModule module, CustomAttributeHandleCollection attributeHandles)
        {
            ...
            foreach (CustomAttributeHandle caHandle in attributeHandles)
            {
                ...
                try
                {
                    ...
                    caDependencies.Add(factory.ScannedMethod(constructor), "Attribute constructor");
                    ...
                    if (AddDependenciesFromCustomAttributeBlob(caDependencies, factory, (EcmaType)constructor.OwningType.GetTypeDefinition(), decodedValue))
                    {
        ...
        private static bool AddDependenciesFromCustomAttributeBlob(DependencyList dependencies, ILScanNodeFactory factory, EcmaType attributeType, CustomAttributeValue<TypeDesc> value)
        {
            MetadataManager mdManager = factory.MetadataManager;
        ...
        private static bool AddDependenciesFromPropertySetter(DependencyList dependencies, ILScanNodeFactory factory, EcmaType attributeType, string propertyName)
        {
            MetadataReader reader = attributeType.MetadataReader;
            var typeDefinition = reader.GetTypeDefinition(attributeType.Handle);
            ...
            foreach (PropertyDefinitionHandle propDefHandle in typeDefinition.GetProperties())
            {
                ...
                if (reader.StringComparer.Equals(propDef.Name, propertyName))
                {
                    ...
                    {
                        MethodDesc setterMethod = (MethodDesc)attributeType.EcmaModule.GetObject(accessors.Setter);
                        if (factory.MetadataManager.IsReflectionBlocked(setterMethod))
                            return false;

                        dependencies.Add(factory.ScannedMethod(setterMethod), "Custom attribute blob");
                    }
            ...
            if (baseType != null)
                return AddDependenciesFromPropertySetter(dependencies, factory, (EcmaType)baseType.GetTypeDefinition(), propertyName);

            // Not found. This is bad metadata that will result in a runtime failure, but we shouldn't fail the compilation.
            return true;
        }

        private static bool AddDependenciesFromCustomAttributeArgument(DependencyList dependencies, ILScanNodeFactory factory, TypeDesc type, object value)
        {
